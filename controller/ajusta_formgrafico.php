...
require_once("../dompdf/dompdf/dompdf_config.inc.php");
	...
	$stmt30 = $dbh->prepare('SELECT token FROM cliente');
    $stmt30->execute();
	
    foreach ($stmt30->fetchAll(PDO::FETCH_ASSOC) as $todos_clientes) {

		$tokenForm='35c0ecc629deeaa5bcced10ed676f7ee';
        $tokenCliente=$todos_clientes['token'];
		
		$stmt10 = $dbh->prepare('SELECT COUNT(id) as validaFormulario FROM resposta WHERE token_cliente=:cliente AND token_formulario=:formulario');
        $stmt10->bindParam(":cliente", $tokenCliente, PDO::PARAM_STR);
        $stmt10->bindParam(":formulario", $tokenForm, PDO::PARAM_STR);
        $stmt10->execute();
        $validacao=$stmt10->fetch();
